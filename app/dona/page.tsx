...
export default function DonaPage() {
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-24 sm:py-20 space-y-20">
      {/* HEADER */}
      <header className="text-center max-w-3xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-center gap-6 mt-8 text-xs text-[#abb3bf]">
          <span className="flex items-center gap-1.5">
            <ShieldCheck className="w-4 h-4 text-[#D42365]" />
          ...
          {projects.map((project) => (
            ...
            >
              <div>
                ...
                <p className="text-xs sm:text-sm text-[#abb3bf] leading-relaxed mb-4">
                  {project.excerpt}
                </p>
                <div className="py-2.5 px-3.5 rounded-xl bg-white/5 text-xs text-[#abb3bf] flex items-center justify-between">
                  <span>Presupuesto objetivo:</span>
                  <span className="text-white font-semibold">{project.indicators.budget} <span className="text-[10px] text-[#abb3bf]/60">(mockup)</span></span>
                </div>
              </div>
              ...
              <div className="pt-4 mt-4 border-t border-white/5 flex items-center justify-between">
                <Link
                  ...
                  className="text-xs text-[#D42365] hover:text-[#f472b6] font-medium inline-flex items-center"
                >
                  Conocer el proyecto
                  <ArrowRight className="w-3.5 h-3.5 ml-1" />
                </Link>
                <span className="text-[11px] text-[#abb3bf]/70">
      ...
      <section className="space-y-6 pt-4">
        <div className="flex items-center gap-2.5">
          <HelpCircle className="w-5 h-5 text-[#D42365]" />
          <h2 className="font-display text-3xl sm:text-4xl font-normal text-white">
          ...
          <Accordion type="single" collapsible className="w-full">
            {donationFaqs.map((faq) => (
              <AccordionItem key={faq.id} value={faq.id}>
                ...
                <AccordionContent className="text-sm leading-relaxed text-[#C4CEDC]">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
